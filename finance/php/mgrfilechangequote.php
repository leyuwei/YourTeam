...
$query_statement = "UPDATE records SET checkerquote='" . $_POST['checkerquote'] . "' WHERE id=" . $_POST['id'] . ";";
$db_query = $conn -> query($query_statement);

if ($db_query) {
    echo "{\"result\": \"1\", \"content\": \"发票简明商品名更改成功\"}";
} else {
    echo "{\"result\": \"0\", \"content\": \"发票简明商品名更改时出现错误\"}";
}
