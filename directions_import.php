...
$mapCandidates = [
    'id' => ['id', '方向编号', 'Direction ID'],
    'title' => ['title', '研究方向', 'Title'],
    'description' => ['description', '方向描述', 'Description'],
...
try {
    ...
    $updateStmt = $pdo->prepare('UPDATE research_directions SET title=?, description=?, bg_color=? WHERE id=?');
    $insertStmt = $pdo->prepare('INSERT INTO research_directions(title, description, bg_color, sort_order) VALUES (?,?,?,?)');
    for ($i = 1; $i < count($rows); $i++) {
        ...
        if ($id !== '') {
            $id = (int)$id;
            $updateStmt->execute([$title, $description, $bgColor, $id]);
            if ($updateStmt->rowCount() > 0) {
                $updated++;
                continue;
            }
        }
        $insertStmt->execute([$title, $description, $bgColor, $nextOrder++]);
        $inserted++;
    }
    $pdo->commit();
} catch (Throwable $e) {
    $pdo->rollBack();
    response_json(['success' => false, 'message' => 'Failed to import directions.'], 500);
}
